import { buildItx, buildMultichainReadonlyClient, buildTokenMapping, deployment, encodeBridgingOps, getTokenAddressForChainId, initKlaster, klasterNodeHost, loadSafeV141Account, rawTx, singleTx } from "klaster-sdk";
...
async function setUpKlaster() {
    ...
    const mcClient = buildMultichainReadonlyClient([sepolia, arbitrumSepolia].map(x => {
        return {
            chainId: x.id,
    ...
    const intersectTokenAndClients = (token, mcClient) => {
