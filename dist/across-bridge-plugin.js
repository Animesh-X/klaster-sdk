...
async function getAcrossSuggestedFees(data) {
    const client = axios.create({
        baseURL: 'https://app.across.to/api/'
    });
    const res = await client.get(`suggested-fees?inputToken=${data.sourceToken}&outputToken=${data.destinationToken}&
    originChainId=${data.sourceChainId}&destinationChainId=${data.destinationChainId}&amount=${data.amount}`);
    return res.data;
}
function encodeAcrossCallData(data, fees) {
    const abi = parseAbi([
...
export const acrossBridgePlugin = async (data) => {
